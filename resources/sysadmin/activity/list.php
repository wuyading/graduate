        ...
        <div class="page-content">
            ...
            <div class="row">
                <div style="margin: 15px 15px 15px 0;">
                    <a class="btn btn-danger" href="<?= toRoute('activity/add') ?>">添加活动剪影</a>
                </div>
                    ...
                    <table class="table table-hover">
                        ...
                        <?php if ($list): foreach ($list as $row): ?>
                            <tr>
                                ...
                                <td><?=$row['sortid']?></td>
                                <td>
                                    <a class="btn btn-primary" href="<?= toRoute('activity/add/' . $row['id']) ?>">修改</a>
...
<script type="text/javascript">
    function delete_row(id) {
        layer.alert('确定删除吗？', {
            ...
            ,yes: function(index){
                layer.close(index);
                $.post('<?=toRoute('activity/ajax_delete_activity')?>',{'id':id},function (res) {
                    if(res.status == 1001){
                        layer.alert(res.msg, {
                            icon: 6
                            ,time: 0 //不自动关闭
